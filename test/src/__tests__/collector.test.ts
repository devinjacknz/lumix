...
describe('MetricsCollector', () => {
  ...
  describe('事件发射', () => {
    test('应该在创建指标时发射事件', (done) => {
      collector.once('metricCreated', (name) => {
    ...
    test('应该在记录值时发射事件', (done) => {
      ...
      collector.once('valueRecorded', (data) => {
        expect(data.name).toBe('test_event');
        expect(data.value.value).toBe(1);
        done();
      });

      collector.record('test_event', 1);
    });

    test('应该在清理指标时发射事件', (done) => {
      collector.once('metricsCleared', () => {
        expect(collector.getAllMetrics().size).toBe(0);
        done();
      });
