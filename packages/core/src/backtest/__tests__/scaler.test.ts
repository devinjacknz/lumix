...
describe('Scaler', () => {
  ...
  // Helper function to convert 1D array to 2D
  const to2D = (arr: number[]): number[][] => arr.map(x => [x]);

  describe('Min-Max Scaling', () => {
    const scaler = new Scaler({ type: ScalingType.MIN_MAX });

    test('should scale 1D array correctly', () => {
      const result = scaler.fitTransform(to2D(testData1D)).map(row => row[0]);
    ...
    test('should scale 2D array correctly', () => {
      const result = scaler.fitTransform(testData2D);
    ...
    test('should handle custom feature range', () => {
      ...
      const result = customScaler.fitTransform(to2D(testData1D)).map(row => row[0]);
  ...
  describe('Error Handling', () => {
    ...
    test('should handle arrays with zeros', () => {
      const scaler = new Scaler({ type: ScalingType.STANDARD });
      const result = scaler.fitTransform(to2D([0, 0, 0])).map(row => row[0]);
